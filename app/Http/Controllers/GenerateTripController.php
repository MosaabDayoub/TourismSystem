...
class GenerateTripController extends Controller
{
    ...
    public function generate(Request $request)       // A function for generate the trip
    {
        try {
            ...
            $requiredFields = ['user_id','fromcity','tocountry','N.days','N.people','preferedplaces','preferedfood','PriceIsImportant','date','totalBudget','places'
            ];
            $missingFields = [];
            ...
            for ($i = 1 ; $i <= $numberOfDays ;$i++) {         // loop for every day in the trip
                ...
                if ($i == 1) {        // Day1
                    ...
                    $graph = new Graph();
                    $graph1 = CustomGraph::buildGraph($places_day, $graph, false, null, $PriceIsImportant, $Cityday, $selectedplaces, $resturantofday);      // create A custom graph which contain the Possible paths for USER:
                    ...
                    if(($i % $city1NumberOfDays == 1) || (($days % $cityNumberOfDays == 0 && $days != 0) && $i > $city1NumberOfDays)) { // check if we have to change city
                        ...
                        $graph = new Graph();
                        $graph1 = CustomGraph::buildGraph($places_day, $graph, $changecity, $hotelAttributes, $PriceIsImportant, $Cityday, $selectedplaces, $resturantofday);      // create A custom graph which contain the Possible paths for USER:
                        $sourceNode = $graph1->getVertex(0);
                ...
                $cost = $ticketprice * $numberOfPeople;                     // calculate the total cost of all nodes
                ...
                $trip_days["day_" . $i]['neededMony'] = ceil($cost * $numberOfPeople) ;
                $Totalcost += $cost * $numberOfPeople;
